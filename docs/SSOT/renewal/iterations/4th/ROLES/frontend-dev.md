...
| **모델 (기본)** | sonnet. **리팩토링 관련 큰 업무** 시 Team Lead가 스폰 시 `model: "opus"` 지정 ([1-project.md §7.2](../1-project.md#72-리팩토링큰-업무-시-모델-선택-backend-dev-frontend-dev)) |

---

## 2. 실행 단위 로딩 (권장)

Task **1건** 구현 시작 시 컨텍스트에 포함 권장: ① task-X-Y-N.md(해당 Task) ② [2-architecture.md](../2-architecture.md) § 프론트엔드 ③ (선택) phase-X-Y-status.md.  
