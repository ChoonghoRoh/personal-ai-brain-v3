...
| 2    | [web-user-test-setup-guide.md](references/web-user-test-setup-guide.md)대로 환경 구축 (Base URL, 백엔드 기동)                 |
...
| **방안 B: 페르소나**    | 위와 동일 + [personas.md](references/personas.md) + 관점별 prompt를 함께 전달해 **기획자/개발자/UI·UX** 관점으로 기록 지시                           | 방안 A와 동일하게 Cursor에서 지시. [방안 B 상세](#방안-b-페르소나-기반-테스트-personasmd-참고) 참고 |
...
- **E2E 스펙이 없는 phase**: 스크립트가 “E2E 스펙 없음” 안내를 출력하고, [방안 A(MCP)](#방안-a-mcpcursor-테스트) 또는 [방안 B(페르소나)](#방안-b-페르소나-기반-테스트-personasmd-참고)로 테스트하라고 안내합니다. - **E2E 스펙이 없을 경우**: 먼저 **E2E 스펙 파일을 작성**한 후 테스트를 진행하거나, [방안 A/B](#0-테스트-실행-방법-요약-3가지-방안)의 대체 테스트 방법(curl API 호출, HTTP 상태 확인 등)을 사용합니다.
  ...
  - **상세 절차**: [integration-test-guide.md](integration-test-guide.md#4-e2e-spec-파일-워크플로우) 섹션 4 "E2E Spec 파일 워크플로우" 참조
    **전제:** [web-user-test-setup-guide.md](references/web-user-test-setup-guide.md)대로 백엔드(Base URL)가 기동 중이어야 E2E가 정상 동작합니다.
...
| 항목          | 내용                                                                                                                                                                                                                                                      |
| ------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **전제**      | [web-user-test-setup-guide.md](references/web-user-test-setup-guide.md) 3.3절 — 백엔드 Base URL 기동, Cursor와 브라우저 MCP가 같은 호스트에서 동작하면 `http://localhost:8001` 접근 가능                                                                  |
| **수행 절차** | 1) 테스트 계획 + 웹 체크리스트 문서를 에이전트에게 전달<br>2) "phase-X-Y 웹 체크리스트를 가상 브라우저에서 순서대로 수행해 줘"처럼 지시<br>3) 필요 시 [personas.md](references/personas.md) + 관점별 prompt를 함께 전달해 **어떤 관점으로 기록할지** 지정 |
| **참고 문서** | [web-user-test-setup-guide.md](references/web-user-test-setup-guide.md) — MCP 브라우저 사용 가능 여부·Base URL 조건<br>**[MCP Cursor 테스트 수행 가이드](references/mcp-cursor-test-guide.md)** — Cursor에서 브라우저 활성화·수행 절차·지시문·문제 해결   |
