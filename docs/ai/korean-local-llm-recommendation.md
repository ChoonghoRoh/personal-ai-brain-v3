...
- Docker·n8n·Backend와 함께 돌릴 때는 **2.4B/3.5B** 크기 사용 권장. **추론·의미 기반 키워드 추천**에는 상대적으로 약하다는 피드백이 있음 → 8GB 대안은 [8gb-inference-and-keyword-alternatives.md](8gb-inference-and-keyword-alternatives.md) 참고.
...
- **추론·의미 추적**이 중요한 키워드/라벨 추천에는 **qwen2.5:3b**를 우선 시험해 볼 것을 권장. 상세·대안(외부 API, 하이브리드)은 [8gb-inference-and-keyword-alternatives.md](8gb-inference-and-keyword-alternatives.md) 참고.
...
- **exaone3.5:2.4b**는 추론·의미 기반 키워드 추천에 약하다는 판단이 있는 경우:  
  → [8gb-inference-and-keyword-alternatives.md](8gb-inference-and-keyword-alternatives.md) 참고.  
...
Ollama를 **12GB+ 전용 서버**로 분리할 때, Backend는 `OLLAMA_BASE_URL=http://원격서버IP:11434`, `OLLAMA_MODEL=모델명`으로 연결합니다. **특정 하드웨어(i5-8400, 32GB RAM, M.2 256GB, GPU Radeon R7 270X 2GB)에 맞춘 OS·서버·Ollama·모델 설계**(CPU 위주, GPU 미사용 권장)는 [llm-dedicated-server-design.md](llm-dedicated-server-design.md) 참고.
...
Backend API·키워드 추천은 Ollama로 통일되어 있으나, **로컬 스크립트**(`search_and_query.py`, `embed_and_store.py`)에서는 gpt4all(GGUF)을 직접 사용할 수 있습니다. Docker 환경에는 gpt4all이 부적합해 Ollama로 대체한 상태입니다. **i5-8400 / 32GB RAM / R7 270X 2GB** 같은 스펙에서는 [llm-dedicated-server-design.md](llm-dedicated-server-design.md)의 Ollama·qwen2.5:7b / EEVE-Korean 10.8B 구성을 우선 권장하고, gpt4all은 호스트에서 별도 실험용으로 사용할 수 있습니다.
...
- 8GB 추론·키워드 대안: [8gb-inference-and-keyword-alternatives.md](8gb-inference-and-keyword-alternatives.md)
- LLM 단독 서버 설계 (OS·서버·모델): [llm-dedicated-server-design.md](llm-dedicated-server-design.md)
