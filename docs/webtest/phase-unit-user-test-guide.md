...
| 2    | [web-user-test-setup-guide.md](web-user-test-setup-guide.md)대로 환경 구축 (Base URL, 백엔드 기동)                            |
...
| **방안 B: 페르소나**    | 위와 동일 + [personas.md](personas.md) + 관점별 prompt를 함께 전달해 **기획자/개발자/UI·UX** 관점으로 기록 지시                                      | 방안 A와 동일하게 Cursor에서 지시. [방안 B 상세](#방안-b-페르소나-기반-테스트-personasmd-참고) 참고 |
...
| `python scripts/webtest.py 11-4 start` | Phase 11-4 통합 테스트 (E2E 스펙 없음 → [phase-11-4-user-test-plan](phase-11-4/phase-11-4-user-test-plan.md) 대체 테스트) |
| `python scripts/webtest.py 11-5 start` | Phase 11-5 Phase 10 고도화·회귀 (E2E 스펙 없음 → [phase-11-5-user-test-plan](phase-11-5/phase-11-5-user-test-plan.md) 대체·Phase 10 E2E 활용) |
...
- **E2E 스펙이 없는 phase**: 스크립트가 “E2E 스펙 없음” 안내를 출력하고, [방안 A(MCP)](#방안-a-mcpcursor-테스트) 또는 [방안 B(페르소나)](#방안-b-페르소나-기반-테스트-personasmd-참고)로 테스트하라고 안내합니다. - **E2E 스펙이 없을 경우**: 먼저 **E2E 스펙 파일을 작성**한 후 테스트를 진행하거나, [방안 A/B](#0-테스트-실행-방법-요약-3가지-방안)의 대체 테스트 방법(curl API 호출, HTTP 상태 확인 등)을 사용합니다.
  - **E2E 스펙 작성 가이드**: 기존 spec 파일(`e2e/phase-9-3.spec.js`, `e2e/phase-10-1.spec.js` 등)을 참조하여 Playwright 기반 테스트 시나리오를 작성합니다.
  - **상세 절차**: [integration-test-guide.md](../devtest/integration-test-guide.md#4-e2e-spec-파일-워크플로우) 섹션 4 "E2E Spec 파일 워크플로우" 참조
    **전제:** [web-user-test-setup-guide.md](web-user-test-setup-guide.md)대로 백엔드(Base URL)가 기동 중이어야 E2E가 정상 동작합니다.
...
- Phase 목록·상태는 [docs/phases/phase-9-navigation.md](../phases/phase-9-navigation.md) 등을 참조할 수 있습니다.
- `docs/webtest/` 아래에 해당 phase용 폴더가 있는지 확인합니다. (예: `docs/webtest/phase-9-1/`, `docs/webtest/phase-9-3/`)

---

## 2. 해당 Phase 폴더 및 문서 확인

- **테스트 계획 문서**: `docs/webtest/phase-<X>-<Y>/phase-<X>-<Y>-user-test-plan.md`
  - 해당 phase의 테스트 목표, 범위, 참고 문서(웹 체크리스트·API 체크리스트) 링크가 있습니다.
- **웹 시나리오 체크리스트**: `docs/phases/phase-<X>-<Y>/phase-<X>-<Y>-web-user-checklist.md`
  - 예: Phase 9-1 → [docs/phases/phase-9-1/phase-9-1-web-user-checklist.md](../phases/phase-9-1/phase-9-1-web-user-checklist.md)
...
| 항목          | 내용                                                                                                                                                                                                                                           |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **전제**      | [web-user-test-setup-guide.md](web-user-test-setup-guide.md) 3.3절 — 백엔드 Base URL 기동, Cursor와 브라우저 MCP가 같은 호스트에서 동작하면 `http://localhost:8000` 접근 가능                                                                  |
| **수행 절차** | 1) 테스트 계획 + 웹 체크리스트 문서를 에이전트에게 전달<br>2) "phase-X-Y 웹 체크리스트를 가상 브라우저에서 순서대로 수행해 줘"처럼 지시<br>3) 필요 시 [personas.md](personas.md) + 관점별 prompt를 함께 전달해 **어떤 관점으로 기록할지** 지정 |
| **참고 문서** | [web-user-test-setup-guide.md](web-user-test-setup-guide.md) — MCP 브라우저 사용 가능 여부·Base URL 조건<br>**[MCP Cursor 테스트 수행 가이드](mcp-cursor-test-guide.md)** — Cursor에서 브라우저 활성화·수행 절차·지시문·문제 해결              |
...
### 방안 B: 페르소나 기반 테스트 ([personas.md](personas.md) 참고)

**[personas.md](personas.md)**에 정의된 **일반 웹 사용자** 기준과 **관점(기획자/개발자/UI·UX 디자이너)**에 맞춰 체크리스트를 수행·기록하는 방식입니다. 수동 테스트 또는 방안 A(MCP)와 함께 사용합니다.

| 항목              | 내용                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **기준 페르소나** | [personas.md](personas.md) 1절 — **일반 웹 사용자**: 메뉴 진입 → 화면 동작 → 결과 확인이 자연스러운지 검증                                                                                                                                                                                                                                                                                                  |
| **관점 지정**     | [personas.md](personas.md) 2절 — **꼼꼼한 기획자** / **개발자** / **UI·UX 디자너** 중 하나 선택 후, 해당 관점에서 발견 사항 기록                                                                                                                                                                                                                                                                            |
| **수행 절차**     | 1) 테스트 계획 + 웹 체크리스트 + **personas.md** + **관점별 prompt**([prompts/prompt-planner.md](prompts/prompt-planner.md), [prompts/prompt-developer.md](prompts/prompt-developer.md), [prompts/prompt-uiux-designer.md](prompts/prompt-uiux-designer.md)) 준비<br>2) 선택한 관점으로 체크리스트 시나리오 수행<br>3) 발견 사항을 해당 관점의 초점(요구사항·플로우 / 버그·API / 시인성·접근성)에 맞춰 기록 |
| **지시문 예시**   | [personas.md](personas.md) 3.2절 — 기획자/개발자/UIUX 관점별 지시문 예시 참조                                                                                                                                                                                                                                                                                                                               |
...
| 항목          | 내용                                                                                                                                                                                                                                                                                                                                           |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **전제**      | [web-user-test-setup-guide.md](web-user-test-setup-guide.md) 3.2절 — `baseURL: 'http://localhost:8000'`, 필요 시 `webServer`로 테스트 전 백엔드 기동                                                                                                                                                                                           |
...
- **순차 가이드**: [phase-10-test-scenario-guide.md](phase-10-test-scenario-guide.md) — E2E → 시나리오 .md 확인 → 실행 → 결과 기록.
- **Phase 10-1 시나리오 .md**: [phase-10-1-mcp-webtest-scenarios.md](phase-10-1/phase-10-1-mcp-webtest-scenarios.md) (Task 10-1-1·10-1-2·10-1-3 각 10개, 총 30개).
- **Phase 10-2 시나리오 .md**: [phase-10-2-mcp-webtest-scenarios.md](phase-10-2/phase-10-2-mcp-webtest-scenarios.md) (Task 10-2-1~10-2-4 각 10개, 총 40개).
...
2. **체크리스트(시나리오)**대로 선택한 방안(A/B/C)으로 실행합니다. (Base URL·환경은 [web-user-test-setup-guide.md](web-user-test-setup-guide.md) 참조)
3. **방안 B**를 사용할 때는 [personas.md](personas.md)에 맞춰 **일반 웹 사용자** 관점으로 동작을 검증하고, 관점별 프롬프트로 발견 사항을 기록합니다.
...
- 예: Phase 9-3 — [phase-9-3/reports/README.md](phase-9-3/reports/README.md), [phase-9-3-final-summary.md](phase-9-3/phase-9-3-final-summary.md) 참조
...
**템플릿**: [\_template/phase-X-Y-user-test-plan.md](_template/phase-X-Y-user-test-plan.md)를 복사한 뒤 phase 번호와 이름을 치환해 사용할 수 있습니다.
